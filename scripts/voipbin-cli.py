...
def get_voipbin_images_from_compose(project_dir="."):
    """Extract voipbin/* images from docker-compose.yml"""
    ...
    try:
        with open(compose_file, "r") as f:
            for line in f:
                line = line.strip()
                if line.startswith("image:") and "voipbin/" in line:
...
""")
    ...
    def _update_images(self, project_dir, check_only=False):
        ...
        # Get list of voipbin images
        images = get_voipbin_images_from_compose(project_dir)
        if not images:
        ...
        override_content = self._generate_override_content(resolved, warnings)
    ...
    def _generate_override_content(self, resolved, warnings):
        ...
        # Add resolved images
        for r in sorted(resolved, key=lambda x: x["image"]):
            # Extract service name from image (e.g., voipbin/bin-api-manager -> api-manager)
            image_name = r["image"]
            if image_name.startswith("voipbin/bin-"):
                service_name = image_name.replace("voipbin/bin-", "")
            elif image_name.startswith("voipbin/"):
                service_name = image_name.replace("voipbin/", "")
            else:
                service_name = image_name.split("/")[-1]

            lines.append(f"  {service_name}:")
            lines.append(f"    image: {image_name}:{r['tag']}")
